...
func NewDiskQueue(name string, maxMsgSize int32) *DiskQueue {
	d := DiskQueue{
		name:              name,
		dataPath:          State.Config.DataPath,
		maxBytesPerFile:   unit.MB * 100,
		minMsgSize:        0,
...
func (d *DiskQueue) exit(deleted bool) error {
	...
	if deleted {
		log.Infof("DISKQUEUE(%s): deleting", d.name)
	} else {
		log.Infof("DISKQUEUE(%s): closing", d.name)
	}
...
func (d *DiskQueue) Empty() error {
	...
	log.Infof("DISKQUEUE(%s): emptying", d.name)
...
func (d *DiskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		log.Infof("DISKQUEUE(%s): readOne() opened %s", d.name, curFileName)
...
func (d *DiskQueue) writeOne(data []byte) error {
	...
	if d.writeFile == nil {
		...
		log.Infof("DISKQUEUE(%s): writeOne() opened %s", d.name, curFileName)
...
func (d *DiskQueue) retrieveMetaData() error {
	...
	if d.writePos < fileSize {
		log.Warnf("DISKQUEUE(%s) %s metadata writePos %d < file size of %d, skipping to new file", d.name, fileName, d.writePos, fileSize)
		d.writeFileNum += 1
		d.writePos = 0
		if d.writeFile != nil {
			d.writeFile.Close()
...
func (d *DiskQueue) moveForward() {
	...
	if oldReadFileNum != d.nextReadFileNum {
		...
		err := os.Remove(fn)
		if err != nil {
			log.Errorf("DISKQUEUE(%s) failed to Remove(%s) - %s", d.name, fn, err)
		}
	}
...
func (d *DiskQueue) handleReadError() {
	...
	log.Warnf("DISKQUEUE(%s) jump to next file and saving bad file as %s", d.name, badRenameFn)

	err := os.Rename(badFn, badRenameFn)
	if err != nil {
		log.Errorf("DISKQUEUE(%s) failed to rename bad diskqueue file %s to %s", d.name, badFn, badRenameFn)
	}
	...
	for {
		...
		if (d.readFileNum < d.writeFileNum) || (d.readPos < d.writePos) {
			if d.nextReadPos == d.readPos {
				...
				if err != nil {
					log.Errorf("DISKQUEUE(%s) reading at %d of %s - %s",
...
exit:
	log.Infof("DISKQUEUE(%s): closing ... ioLoop", d.name)
	syncTicker.Stop()
	d.exitSyncChan <- 1
}
